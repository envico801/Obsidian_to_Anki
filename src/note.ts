	...
	parse(
		...
		if (url) {
			this.formatter.format_note_with_url(
				...
				data.prepend_file_link_line_break,
			);
		}
...
export class RegexNote {
	...
	constructor(
		...
		this.tags =
			tags ? this.match.pop().slice(TAG_PREFIX.length).split(TAG_SEP) : [];
	...
	getFields(): Record<string, string> {
		...
		for (let index in this.match.slice(1)) {
			fields[this.field_names[index]] =
				this.match.slice(1)[index] ? this.match.slice(1)[index] : '';
		}
		for (let key in fields) {
			fields[key] = this.formatter
				.format(
