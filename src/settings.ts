...
const defaultDescs = {
	...
	'Prepend File Link Line Break':
		'Prepends a line break to the File Link field. This toggle defaults to true to preserve the historically expected affect of this field. Toggling false will not update existing flashcards, only newly generated ones.',
	'Add Context':
...
export class SettingsTab extends PluginSettingTab {
		...
		let link_field = new Setting(row_cells[2] as HTMLElement).addDropdown(
			async (dropdown) => {
				if (!plugin.fields_dict[note_type]) {
					...
					if (
						Object.keys(plugin.fields_dict).length != plugin.note_types.length
					) {
						new Notice(
	...
	setup_ignore_files() {
		...
		plugin.settings['IGNORED_FILE_GLOBS'] =
			plugin.settings.hasOwnProperty('IGNORED_FILE_GLOBS') ?
				plugin.settings['IGNORED_FILE_GLOBS']
