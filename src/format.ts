...
export class FormatConverter {
	...
	format_note_with_url(
		...
		note.fields[field] +=
			'<a href="' + url + '" class="obsidian-link">Obsidian</a>';
	}
	...
	format(
		...
		const add_highlight_css: boolean =
			note_text.match(c.OBS_DISPLAY_CODE_REGEXP) ? true : false;
		...
		if (add_highlight_css) {
			note_text =
				'<link href="' + c.CODE_CSS_URL + '" rel="stylesheet">' + note_text;
		}
		return note_text;
	}
}
